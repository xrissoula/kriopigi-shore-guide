/** Greek translations for the Biogeochemistry page (Field Notes II · Deep Time). */
export const elBiogeochemistry: Record<string, string> = {
  // --- Page header ---
  "Deep Time · II": "Βαθύς Χρόνος · II",
  "Biogeochemistry of the Aegean": "Βιογεωχημεία του Αιγαίου",
  "From a subducting plate to dissolved oxygen, five layers of process that meet at the Kriopigi shore.":
    "Από την καταβύθιση μιας λιθοσφαιρικής πλάκας έως το διαλυμένο οξυγόνο, πέντε επίπεδα διεργασιών που διαμορφώνουν τις ακτές της Κρυοπηγής.",
  "Back to Geology": "Επιστροφή στη Γεωλογία",

  // --- Continue card ---
  "Continue · II · Human Time": "Συνέχεια · II · Ανθρώπινος Χρόνος",
  "Tired of the geology? How about archaeology?": "Αρκετή γεωλογία· πάμε στην ιστορία;",
  "Click here to learn about the Anthropological History of Kriopigi.":
    "Πατήστε εδώ για να εξερευνήσετε την ανθρώπινη ιστορία της Κρυοπηγής.",
  "The fortified arsanas (sea-gate tower) of the Great Lavra monastery":
    "Το οχυρωμένο αρσανάς (πύργος της θαλάσσιας πύλης) της Μονής Μεγίστης Λαύρας",

  // --- Stages ---
  "Tectonic foundation": "Το τεκτονικό υπόβαθρο",
  "A subducting plate beneath the sea": "Μια λιθοσφαιρική πλάκα που καταβυθίζεται κάτω από τη θάλασσα",
  "The Aegean sits above the Hellenic Subduction System, where the African plate dives beneath Eurasia along a curved trench south of Crete. This slow convergence drives the tectonic evolution of the region, creating the basins, volcanic arc, and seafloor topography that in turn influence circulation and the chemistry of the water column.":
    "Το Αιγαίο βρίσκεται πάνω από το Ελληνικό Σύστημα Καταβύθισης, όπου η αφρικανική λιθοσφαιρική πλάκα βυθίζεται κάτω από την ευρασιατική κατά μήκος της καμπύλης τάφρου νότια της Κρήτης. Αυτή η αργή σύγκλιση οδηγεί την τεκτονική εξέλιξη της περιοχής, δημιουργώντας τις λεκάνες, το ηφαιστειακό τόξο και την υποθαλάσσια μορφολογία, που με τη σειρά τους επηρεάζουν την κυκλοφορία και τη χημεία των υδάτων.",
  "Map of the Hellenic Subduction System in the Eastern Mediterranean showing trenches, earthquakes and mud volcanoes":
    "Χάρτης του Ελληνικού Συστήματος Καταβύθισης στην Ανατολική Μεσόγειο, με τάφρους, σεισμούς και ηφαιστεία λάσπης",
  "The Hellenic Subduction System in the Eastern Mediterranean. After Mouslopoulou et al. (2025), Tectonics 44, e2025TC008943.":
    "Το Ελληνικό Σύστημα Καταβύθισης στην Ανατολική Μεσόγειο. Από τους Mouslopoulou και συν. (2025), Tectonics 44, e2025TC008943.",

  "Back-arc extension": "Εφελκυσμός του οπισθότοξου",
  "Graben, volcanoes, and a stretched crust": "Τεκτονικές τάφροι, ηφαίστεια και εκτεταμένος φλοιός",
  "Behind the arc, the Aegean crust pulls apart. The North Aegean and Skyros basins open as tectonic graben, while the volcanic arc, Methana, Milos, Santorini, Nisyros, punches through the thinned crust. This extensional architecture largely determines the depth and geometry of the basins that guide modern water circulation.":
    "Πίσω από το Ελληνικό Τόξο, ο φλοιός του Αιγαίου διατείνεται και λεπταίνει. Οι λεκάνες του Βόρειου Αιγαίου και της Σκύρου σχηματίζονται ως τεκτονικές τάφροι (graben), ενώ το ηφαιστειακό τόξο, Μέθανα, Μήλος, Σαντορίνη και Νίσυρος, διαπερνά τον λεπτυσμένο φλοιό. Αυτή η εκτατική δομή διαμόρφωσε το βάθος και τη γεωμετρία των λεκανών μέσα στις οποίες κυκλοφορούν σήμερα οι υδάτινες μάζες του Αιγαίου.",
  "Map of tectonic graben and volcanism in the Aegean, Greece":
    "Χάρτης τεκτονικών ταφρών και ηφαιστειότητας στο Αιγαίο, Ελλάδα",
  "Active tectonic graben and the Aegean volcanic arc. After Papanikolaou, Nomikou & Lampridou (2025), 'Tectonic graben and volcanism in the Aegean, Greece', Geological Society, London, Special Publications 560.":
    "Ενεργές τεκτονικές τάφροι και το ηφαιστειακό τόξο του Αιγαίου. Από τους Papanikolaou, Nomikou & Lampridou (2025), Geological Society, London, Special Publications 560.",

  Bathymetry: "Βαθυμετρία",
  "The basins water settles into": "Οι λεκάνες όπου συγκεντρώνονται οι υδάτινες μάζες",
  "Tectonics and erosion together carved a complex bottom: the deep North Aegean trough, the Athos and Sporades basins, the Chios and Skyros depressions. Halkidiki occupies a shallow continental shelf bordering the Athos Basin, where local circulation reflects interactions between shelf waters, regional currents, and the deeper Aegean basins.":
    "Οι τεκτονικές διεργασίες και η διάβρωση διαμόρφωσαν έναν ιδιαίτερα πολύπλοκο θαλάσσιο πυθμένα: τη βαθιά τάφρο του Βόρειου Αιγαίου, τις λεκάνες του Άθω και των Σποράδων, καθώς και τις βυθίσεις της Χίου και της Σκύρου. Η Χαλκιδική βρίσκεται σε μια ρηχή ηπειρωτική υφαλοκρηπίδα που συνορεύει με τη Λεκάνη του Άθω, όπου η τοπική κυκλοφορία επηρεάζεται από τις αλληλεπιδράσεις ανάμεσα στα νερά της υφαλοκρηπίδας, τα περιφερειακά ρεύματα και τις βαθύτερες λεκάνες του Αιγαίου.",
  "Bathymetric map of the Aegean Sea showing major basins": "Βαθυμετρικός χάρτης του Αιγαίου Πελάγους με τις κύριες λεκάνες",
  "Bottom topography of the Aegean Sea. Map adapted from Karageorgis (1995).":
    "Βαθυμετρία του Αιγαίου Πελάγους. Χάρτης προσαρμοσμένος από τον Karageorgis (1995).",

  "Surface circulation": "Επιφανειακή κυκλοφορία",
  "How water moves through the Aegean": "Πώς κινούνται οι υδάτινες μάζες του Αιγαίου",
  "Cool, fresher Black Sea water enters from the Dardanelles and sweeps west and south along the Greek coast; warmer, saltier Levantine water pushes north along the Turkish side. The interaction between fresher Black Sea water and warmer, saltier Levantine water helps shape seasonal fronts, mixing, and circulation around Halkidiki, flushing coastal bays and influencing the distribution of nutrients.":
    "Οι ψυχρότερες και λιγότερο αλμυρές υδάτινες μάζες της Μαύρης Θάλασσας εισέρχονται από τα Δαρδανέλλια και κινούνται προς τα δυτικά και νότια κατά μήκος των ελληνικών ακτών, ενώ οι θερμότερες και πιο αλμυρές υδάτινες μάζες της Λεβαντίνης ανέρχονται προς τον βορρά κατά μήκος των τουρκικών παραλίων. Η αλληλεπίδραση αυτών των υδάτινων μαζών δημιουργεί ένα πολύπλοκο σύστημα εποχιακών μετώπων, ανάμειξης και κυκλοφορίας σε ολόκληρο το Αιγαίο, το οποίο επηρεάζει και τις ακτές της Χαλκιδικής.",
  "Schematic of Aegean Sea upper circulation": "Σχηματική απεικόνιση της επιφανειακής κυκλοφορίας του Αιγαίου",
  "Schematic representation of the Aegean Sea upper circulation (following Theocharis et al., 1993 & 1999; Theocharis & Georgopoulos, 1993; Zodiatis, 1994; Zervakis & Georgopoulos, 2002).":
    "Σχηματική απεικόνιση της επιφανειακής κυκλοφορίας του Αιγαίου (βασισμένη στους Theocharis κ.ά., 1993, 1999· Theocharis & Georgopoulos, 1993· Zodiatis, 1994· Zervakis & Georgopoulos, 2002).",

  Biogeochemistry: "Βιογεωχημεία",
  "Oxygen, nutrients, and the open Mediterranean": "Οξυγόνο, θρεπτικά στοιχεία και η ανοιχτή Μεσόγειος",
  "Water exchanged through the Cretan Straits links the Aegean to the wider Eastern Mediterranean. These exchanges contribute to the formation and transformation of intermediate and deep water masses, including Levantine Intermediate Water, which plays a major role in ventilating the eastern Mediterranean. The Rhodes Gyre, farther to the southeast, is one of the basin's principal sites of winter deep-water formation and oxygenation. The chemistry of the water at Kriopigi is one node within this much larger circulation.":
    "Το Αιγαίο ανταλλάσσει επιφανειακά, ενδιάμεσα και βαθιά νερά με την υπόλοιπη Ανατολική Μεσόγειο μέσω των στενών της Κρήτης. Πιο νοτιοανατολικά, στον στρόβιλο της Ρόδου, ο χειμερινός σχηματισμός ενδιάμεσων υδάτων και η πρόσληψη οξυγόνου αποτελούν σημαντικές διεργασίες για την ανατολική Μεσόγειο. Η χημεία των νερών στην Κρυοπηγή αποτελεί ένα μικρό μέρος αυτού του ευρύτερου συστήματος κυκλοφορίας.",
  "Map of dissolved oxygen and circulation in the Levantine Sea": "Χάρτης διαλυμένου οξυγόνου και κυκλοφορίας στη Λεκάνη της Λεβαντίνης",
  "Dissolved oxygen and circulation in the Levantine Basin. From Habib et al. (2026), 'Dissolved oxygen budget in the Levantine Sea: a coupled physical-biogeochemical modelling approach', Biogeosciences 23, 2939-2958, https://doi.org/10.5194/bg-23-2939-2026 (CC BY 4.0).":
    "Διαλυμένο οξυγόνο και κυκλοφορία των νερών στη Λεκάνη της Λεβαντίνης. Από τους Habib και συν. (2026), Biogeosciences 23, 2939-2958, https://doi.org/10.5194/bg-23-2939-2026 (CC BY 4.0).",
};
